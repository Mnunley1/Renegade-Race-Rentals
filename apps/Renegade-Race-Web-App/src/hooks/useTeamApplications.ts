...
export function useTeamApplications() {
  ...
  // Get applications by team
  const getApplicationsByTeam = useCallback((teamId: Id<'teams'>) => {
    return useQuery(api.teamApplications.getByTeam, { teamId });
  }, []);

  // Get applications by driver
  const getApplicationsByDriver = useCallback(() => {
