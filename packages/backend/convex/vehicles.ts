...
export const getAllWithOptimizedImages = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (trackId) {
      ...
      vehicles = await ctx.db
        .query('vehicles')
        .withIndex('by_active_approved', (q: any) =>
...
export const getById = query({
  ...
  handler: async (ctx, args) => {
    ...
    const [images, owner, track, availability] = await Promise.all([
      ...
      ctx.db
        .query('availability')
        .withIndex('by_vehicle_date', (q: any) => q.eq('vehicleId', args.id))
        .order('asc')
        .collect(),
    ]);
  ...
  handler: async (ctx, args) => {
    ...
    await Promise.all(
      args.images.map(async (image) => {
        ...
        const [imageUrl, thumbnailUrl, cardUrl, detailUrl, heroUrl] =
          await Promise.all([
            image.storageId
...
export const addImage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existingImages = await ctx.db
      .query('vehicleImages')
      .withIndex('by_vehicle', (q: any) => q.eq('vehicleId', args.vehicleId))
      .collect();
