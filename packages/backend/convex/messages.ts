...
export const send = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (conversation.renterId === senderId) {
      updateData.unreadCountOwner = (conversation.unreadCountOwner || 0) + 1;
    } else {
      updateData.unreadCountRenter = (conversation.unreadCountRenter || 0) + 1;
    }
...
export const markConversationAsRead = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (conversation.renterId === args.userId) {
      updateData.unreadCountRenter = 0;
    } else {
