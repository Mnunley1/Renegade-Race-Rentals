...
export const createPaymentIntent = action({
  ...
  handler: async (ctx, args) => {
    ...
    const reservation = await ctx.runQuery('reservations:getById', {
    ...
    const paymentId = await ctx.runMutation('stripe:createPaymentRecord', {
      reservationId: args.reservationId,
      renterId: reservation.renterId,
      ownerId: reservation.ownerId,
      amount: args.amount,
...
export const confirmPayment = action({
  ...
  handler: async (ctx, args) => {
    const payment = await ctx.runQuery('stripe:getPaymentById', {
    ...
    if (paymentIntent.status === 'succeeded') {
      ...
      await ctx.runMutation('stripe:updatePaymentStatus', {
        paymentId: args.paymentId,
        status: 'succeeded',
      ...
      await ctx.runMutation('stripe:updateReservationStatus', {
...
export const processRefund = action({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.runMutation('stripe:updateRefundStatus', {
      paymentId: args.paymentId,
      status:
    ...
    const paymentsWithDetails = await Promise.all(
      payments.map(async (payment) => {
        const [reservation, vehicle, owner] = await Promise.all([
          ...
          ctx.db.get(payment.metadata?.vehicleId as any),
          ctx.db
            .query('users')
